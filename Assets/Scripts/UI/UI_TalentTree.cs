...
{
    [UIWindowData(typeof(UI_TalentTree),false,"UI_TalentTree",3)]
        ...
        private void CreateLine(GameObject curTalent)
        {
            ...
            if (talentDataConfig.preData.Count != 0)
            {
                for (int i = 0; i < talentDataConfig.preData.Count; i++)
                {
                    ...
                    {
                        Vector2 prePos = currentTalent[talentDataConfig.preData[i].talentName]
                        ...
                        line1.GetComponent<RectTransform>().sizeDelta =
                            new Vector2((curPos.x - prePos.x - 2 * outlineHeight) / 2, lineGap);
                        //创建第二折
                        float x2, y2;
                        ...
                        if (curPos.x != prePos.x)
                        {
                            ...
                            line2.GetComponent<RectTransform>().sizeDelta = new Vector2(lineGap,
                                Mathf.Abs(Mathf.Abs(curPos.y) - Mathf.Abs(prePos.y)) + lineGap);
                        }
                        else
                        {
                            x2 = prePos.x;
                        ...
                        line3.GetComponent<RectTransform>().sizeDelta =
                            new Vector2((curPos.x - prePos.x - 2 * outlineHeight) / 2, lineGap);
